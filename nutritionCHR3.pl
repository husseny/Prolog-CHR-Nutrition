...
:-chr_constraint done/1, schedule/4, week/5, dailyIntake/7, meal/6, food/10.
...
week(Week, Protein, Carb, Fat, NoMeals) <=> 
	bagof(food(A,B,K,D,E,M,G,H,I,J),food1(A,B,K,D,E,M,G,H,I,J),L), 
...
food(X, P, C, F, Prefs, Week, Day, Order, L, 2), 
food(X, P, C, F, Prefs, _, _, _, L, 0) , meal(Week, Day, Protein, Carb, Fat, Order) <=>  
NProtein is Protein - P, NCarb is Carb - C, NFat is Fat - F, NProtein >= -30, NCarb >= -25, NFat >= -5,
not(member((Week,Day), L)), member(Order, Prefs) | 
food(X, P, C, F, Prefs, _, _, _, [(Week,Day)|L], 0),
...
food1(cheddarChs, 25, 1.3, 33, [1,2,5], _, _, _, [], 0).

food1(goudaChs, 25, 2.2, 27, [1,2,5], _, _, _, [], 0).

food1(swissChs, 27, 5, 28, [1,2,5], _, _, _, [], 0).

food1(carrot, 0.9, 10, 0.2, [1,2,3,4,5], _, _, _, [], 0).

food1(broccoli, 2.8, 7, 0.4, [1,2,3,4,5], _, _, _, [], 0).

food1(spinach, 2.9, 3.6, 0.4, [2,3,4,5], _, _, _, [], 0).

food1(whtRice, 7, 82, 0.6, [3,4], _, _, _, [], 0).

food1(potato, 2, 17, 0.1, [3,4], _, _, _, [], 0).

food1(whlEgg, 13, 0.7, 10, [1,2,4,5], _, _, _, [], 0).

food1(eggWht, 11, 0.7, 0.2, [1,2,4,5], _, _, _, [], 0).

food1(swtPotato,	1.6, 20, 0, [1,2,3,4,5], _, _, _, [], 0).

food1(fullFatMilk, 3.3, 4.6, 3.7, [1,2,3,4], _, _, _, [], 0).

food1(pear, 0.4, 15, 0.1, [1,2,3,4,5], _, _, _, [], 0).

food1(tuna, 26, 0, 1, [2,3,4,5], _, _, _, [], 0).

food1(tomato, 0.9, 3.9, 0.2, [1,2,3,4,5], _, _, _, [], 0).

food1(honey,	0.3, 82, 0, [1,2,3,4,5], _, _, _, [], 0).

food1(oliveOil, 0, 0, 100, [1,2,3,4,5], _, _, _, [], 0).

food1(pasta, 13, 75, 1.5, [3,4], _, _, _, [], 0).

food1(salmon, 20, 0, 13, [3,4], _, _, _, [], 0).

food1(fish, 19, 0, 6, [3,4], _, _, _, [], 0).

food1(shrimp, 20, 0, 0.5, [3,4], _, _, _, [], 0).

%food1(skimmedMilk, 3.4, 5, 0.1, [1,2,3,4,5], _, _, _, [], 0).

food1(banana, 1.1, 23, 0.3, [1,2,3,4,5], _, _, _, [], 0).

food1(chkBr, 21.2, 0, 2.5, [3,4], _, _, _, [], 0).

food1(lnBeef, 20, 0, 6, [3,4], _, _, _, [], 0).

food1(peanutButter, 25, 20, 50, [1,2,3,4], _, _, _, [], 0).

food1(wheyProtein, 80, 10, 3.3, [1,2,3,4,5], _, _, _, [], 0).

food1(multiGrainBread, 13, 43, 4.2, [1,2,3,4,5], _, _, _, [], 0).
